...
def print_leaderboard(
    ...
    hdr = f"  {'#':>3}  {'ID':>8}  {'Label':<45}  {'TER':>6}  {'RQS':>6}  {'TV':>6}"
    if has_llm:
    ...
    for rank, r in enumerate(ranked, 1):
        ...
        row = (
            ...
            f"{ter*100:>5.1f}%  {rqs:>6.3f}  {tv:>6.3f}"
        )
        if has_llm:
        ...
        if show_diff:
            ...
            row += f"  {sign_ter}{dter:>4.1f}%  {sign_tv}{dtv:>5.3f}"
        row += f"  {ts:<10}"
        print(row)

    print(f"\n  Total experiments: {len(results)}  |  Shown: {len(ranked)}")
